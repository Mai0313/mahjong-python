...
from .melds import Wu, Chow, Kong, Meld, Pong, WuFlag
from .tiles import Tile, Wind, Honors, Bonuses, Simples, BonusTile
...
STOCK_TABLES = {
    "enwp": {1: 1, 2: 1, 3: 1, 4: 2, 5: 2, 6: 2, 7: 4, 8: 4, 9: 4, 10: 8},
    "zhwp": {
        0: 1,
        1: 2,
        2: 4,
    ...
    # 7 faan = 7200c = 6x1200
    # 8 faan = 9600c = 8x1200
    "random_app": {1: 1, 2: 1, 3: 1, 4: 2, 5: 3, 6: 4, 7: 6, 8: 8, 9: 10, 10: 13},
}
...
class HandEnding(NamedTuple):
    ...
    def points(
        self, min_faan: int = 3, table: Optional[Mapping[int, int]] = None
        ...
        if (
            (WuFlag.TWELVE_PIECE in wu.flags and WuFlag.SELF_DRAW in wu.flags)
            or (WuFlag.GAVE_KONG in wu.flags and WuFlag.SELF_DRAW in wu.flags)
            or WuFlag.GAVE_DRAGON in wu.flags
        ) and wu.discarder is not None:
...
class UserIO(NamedTuple):
    ...
    def answer(self, ans=None):
        """This method is used to provide an answer to a question in the game.

        Parameters:
            ans (optional): The answer to the question. Defaults to None.
    ...
    def __repr__(self) -> str:
        ...
        """
        return (
            f"UserIO(question={self.question}, melds={self.melds}, "
...
class Hand:
    ...
    def play(self, wind: Wind = Wind.EAST) -> Union[UserIO, HandEnding]:
        ...
        if self.round is not None:
            raise RuntimeError("This Hand is part of a Round " "and cannot be played standalone.")
    ...
    def execute(self, wind: int):
        ...
        if WuFlag.SELF_DRAW in ending.wu.flags and ending.winner in self._12_piece:
            ending.wu.base_flags |= WuFlag.TWELVE_PIECE
            ending.wu.discarder = self._12_piece[ending.winner].seat
        if len(ending.wu.melds) > 1:
            question = UserIO(Question.WHICH_WU, self.gen, melds=ending.wu.melds)
            answer = yield question
        else:
            answer = ending.wu.melds[0]
        if ending.winner is self.players[self.wind]:
            return HandEnding(HandResult.DEALER_WON, self, ending.winner, ending.wu, answer)
        return HandEnding(HandResult.NORMAL, self, ending.winner, ending.wu, answer)
...
class Turn:
    ...
    def execute(self, last_ending: TurnEnding, turncount: int):
        ...
        if (
            last_ending.jumped_with is not None
            ...
            if old_count == 3 and len(player.shown) == 4:
                self.hand._12_piece[player] = self.players[last_ending.prev_seat]
            if not old_drag and self.all_dragons(player):
                self.hand._gave_dragon[player] = self.players[last_ending.prev_seat]
            del old_count, old_drag
        elif last_ending.discard is not None and not last_ending.offered:
            # Step 16
            meld: Optional[Meld] = yield from self.melds_from_discard(player, last_ending)
        ...
        if isinstance(meld, Kong) or meld is None:
            ...
            while 1:
                ...
                try:
                    ...
                    if (
                        WuFlag.BY_KONG in flags or WuFlag.DOUBLE_KONG in flags
                    ...
                    question = UserIO(Question.SELF_DRAW_Q, self.gen, [wu], player, drawn)
        ...
        question = UserIO(
            Question.DISCARD_WHAT, self.gen, player=player, arrived=arrived, last_meld=meld
        )
        ...
        if thief is None and isinstance(meld, bool):
            ...
            return TurnEnding(
                discard=answer,
                offered=meld,
                seat=Wind((player.seat + 1) % 4),
                prev_seat=player.seat,
            )
        # Step 31
        if isinstance(meld, Wu):
            return TurnEnding(winner=thief, wu=meld)
        return TurnEnding(discard=answer, seat=thief.seat, jumped_with=meld, prev_seat=player.seat)
    ...
    def melds_from_discard(self, player: Player, last_ending: TurnEnding):
        ...
        try:
            wu = Wu(
                [*player.hand, last_ending.discard],
                player.shown,
                last_ending.discard,
        ...
        if melds:
            # Step 17
            question = UserIO(
                Question.MELD_FROM_DISCARD_Q, self.gen, melds, player, last_ending.discard
            )
    ...
    def check_ekfp(self, draw: Tile, player: Player):
        ...
        if kongs:
            question = UserIO(
                Question.SHOW_EKFCP_Q, self.gen, melds=kongs, player=player, arrived=draw
            )
        ...
        if kongs:
            question = UserIO(
                Question.SHOW_EKFEP_Q, self.gen, melds=kongs, player=player, arrived=draw
            )
            answer: Optional[Kong] = yield question
            if answer is not None:
    ...
    def check_kong_robbers(self, kong: Kong, victim: Player):
        ...
        for p in self.players:
            ...
            try:
                ...
                continue
            question = UserIO(Question.ROB_KONG_Q, self.gen, melds=[wu], player=p)
    ...
    def check_others_melds(self, discard: Tile, victim: Player):
        ...
        for melds in overall.values():
            melds.sort()
        pairs: list[tuple[Player, Meld]] = [(p, melds[0]) for p, melds in overall.items() if melds]
        ...
        for player, meld in pairs:
            ...
            question = UserIO(
                ...
                self.gen,
                melds=overall[player],
                player=player,
                arrived=discard,
            )
            answer: Optional[Meld] = yield question
            if answer is not None:
                if isinstance(answer, Kong) and discard not in self.hand.discarded[:-1]:
